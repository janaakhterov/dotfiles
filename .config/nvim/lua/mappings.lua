...
vim.api.nvim_set_keymap("n", "<leader>rr", [[<Cmd> lua require('telescope.builtin').lsp_references(require('telescope.themes').get_dropdown())<CR>]], options)
vim.api.nvim_set_keymap("n", "<leader>y", [[<Cmd> SidebarNvimToggle<CR>]], options)

vim.keymap.set("n", "<leader>t", function() require("nvim-tree.api").tree.toggle() end, options)
